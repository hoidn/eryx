...
def run_high_res_validation(pump_magnitude=3.0, pump_energy_percentile=95.0):
    """
    Runs a high-resolution visual check comparing the default thermal model
    against a model with a moderately "pumped" phonon mode.
    ...
    pump_energy_percentile : float
        Percentile of frequency distribution to pump (default: 95.0)
    """
    print("--- Starting High-Resolution 'Pumped Mode' PDOS Visual Validation ---")
    ...
    print(f"Initializing a base model with higher resolution (sampling rate: {hsampling_high_res[2]})...")
    ...
    print(f"Creating a 'pumped' thermal PDOS by exciting a mode at {pump_energy_percentile}th percentile...")
    ...
    fig.suptitle("Default Thermal vs. 'Pumped' Phonon Population (High Resolution)", fontsize=18)
    ...
    ax.axvline(actual_pump_freq_thz, color='red', linestyle='--', lw=2, label=f'Pumped Mode ({actual_pump_freq_thz:.2e} THz)')
    ...
    def plot_slice(ax, intensity_map, title, hkl_grid):
        ...
        im = ax.imshow(log_slice_data, origin='lower', cmap='viridis', vmin=vmin, vmax=vmax, 
    ...
    output_filename = f"pdos_pumped_mode_high_res_mag{pump_magnitude:.1f}_pct{pump_energy_percentile:.0f}.png"
...
def main():
    ...
    parser = argparse.ArgumentParser(description='High-resolution PDOS pumped mode validation')
    ...
    parser.add_argument('--pump-energy-percentile', '-p', type=float, default=95.0,
    ...
    if args.list_examples:
        print("Example usage:")
        print("  python validate_pumped_mode_high_res.py                    # Default: 3x pump at 95th percentile")
        print("  python validate_pumped_mode_high_res.py -m 5.0 -p 90      # 5x pump at 90th percentile")
        print("  python validate_pumped_mode_high_res.py -m 1.5 -p 50      # 1.5x pump at median frequency")
        print("  python validate_pumped_mode_high_res.py -m 10.0 -p 99     # Strong 10x pump at highest frequencies")
        sys.exit(0)

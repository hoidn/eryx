...
| **Section 2: Plotting and Visualization Corrections** | | | |
| 2.A | Fix Population Plots to Show Actual Populations | [ ] | **Why:** Currently shows only density of states, not actual phonon populations. This is scientifically misleading. <br> **File:** `pump_probe_validation.py` <br> **How:** Replace the current histogram logic (lines 107-123) to use Winv values as weights: <br> 1. Extract `winv_values_thermal = original_winv.real.flatten().detach().cpu().numpy()` <br> 2. Extract `winv_values_pumped = pumped_winv.real.flatten().detach().cpu().numpy()` <br> 3. Create histograms with `weights=winv_values_thermal[valid_mask]` for thermal <br> 4. Use `weights=winv_values_pumped[valid_mask]` for pumped <br> 5. Use `density=False` to preserve absolute population values <br> 6. Update y-axis labels to "Phonon Population (arb. units)" <br> **Verify:** The pumped plot shows a clear spike at the pumped frequency, representing actual increased population. |
| 2.B | Verify NaN Handling in Plots | [ ] | **Why:** To ensure the NaN fix from Phase 1 is working correctly. <br> **File:** `pump_probe_validation.py` <br> **How:** The fix is already implemented (replacing NaN with vmin). Run the script and visually inspect the output PNG to ensure there are no white/blank regions in the intensity plots. Add a comment in the code documenting this fix. <br> **Verify:** The intensity plots show smooth color gradients without white artifacts. |
| 2.C | Verify Matplotlib Offset Fix | [ ] | **Why:** To ensure the offset notation fix from Phase 1 is working. <br> **File:** `pump_probe_validation.py` <br> **How:** The fix is already implemented. Run the script and check that the PDOS plot x-axes show clean numbers without "1e-13" or similar offset notation. Add a comment documenting this fix. <br> **Verify:** The x-axis labels show clean values like "0.0", "0.5", "1.0" without scientific notation offsets. |
| 2.D | Verify Miller Index Labels | [ ] | **Why:** To ensure the crystallographic labeling is correct. <br> **File:** `pump_probe_validation.py` <br> **How:** The implementation is already in place. Run the script and verify that the intensity plot axes show Miller indices (h, l) instead of pixel coordinates. Add comments explaining the Miller index calculation if not already present. <br> **Verify:** The intensity plots are labeled with Miller indices (e.g., -4 to 4). |
| **Section 3: Enhanced Features** | | | |
| 3.A | Add Gaussian Pump Profile Option | [ ] | **Why:** Current delta function pump is unphysical. Real lasers have finite bandwidth (~0.1-1 THz), creating Gaussian excitation profiles. <br> **File:** `pump_probe_validation.py` <br> **How:** <br> 1. Add CLI arguments: `--pump-type {delta,gaussian}` (default: delta) and `--pump-width` (default: 0.5 THz) <br> 2. Modify pumping logic (lines 100-105) to support both modes: <br> - Delta: current single-mode modification <br> - Gaussian: loop through all modes, add `pump_intensity * exp(-0.5*(Δf/σ)²)` <br> 3. Add cutoff (weight > 0.01) for efficiency <br> 4. Update print statements to show pump type and width <br> **Verify:** Running with `--pump-type gaussian --pump-width 0.5` produces a smooth bump in the population plot instead of a sharp spike. |
| 3.B | Review and Document CLI | [ ] | **Why:** To ensure the CLI is user-friendly and well-documented. <br> **File:** `pump_probe_validation.py` <br> **How:** <br> 1. Review the `--help` output for clarity and completeness <br> 2. Ensure all parameter descriptions are accurate (fix "default: 95.0" in help text that contradicts actual default of 50.0) <br> 3. Document new pump-type and pump-width parameters <br> 4. Add examples showing both delta and Gaussian pumping <br> **Verify:** Running `python pump_probe_validation.py --help` shows clear, accurate information including new pump profile options. |
| 3.C | Test Parameter Ranges | [ ] | **Why:** To ensure the script handles various parameter values correctly. <br> **File:** Command line testing <br> **How:** Test the script with various parameters: <br> - Pump types: `--pump-type delta`, `--pump-type gaussian` <br> - Pump widths: `--pump-width 0.1`, `--pump-width 1.0`, `--pump-width 2.0` <br> - Extreme magnitudes: `-m 0.1`, `-m 10.0` <br> - Different percentiles: `-p 1`, `-p 50`, `-p 99` <br> - Different slice indices: `-s 0`, `-s 2` <br> **Verify:** All parameter combinations produce valid output without errors. Gaussian pump shows broader excitation than delta. |
...
- [ ] **Correct Visualization:** All plots display correctly with proper labels and no artifacts. Population plots show actual phonon populations (weighted by Winv), not just density of states.
- [ ] **Enhanced Features:** Both delta function and Gaussian pump profiles are supported, providing physically realistic laser excitation modeling.
- [ ] **Robust CLI:** The script handles all reasonable parameter inputs gracefully, including pump type and width parameters.
- [ ] **Code Quality:** The final script is well-documented, explaining both what it does and why the new approach is superior.
- [ ] **Scientific Accuracy:** The plots accurately represent phonon populations, and the documentation clearly explains how the direct mode modification is more physically accurate than histogram binning. The Gaussian pump option provides realistic finite-bandwidth laser excitation.
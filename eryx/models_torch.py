...
class OnePhonon:
    ...
    # Note on sampling parameters interpretation:
    # There are two potential ways to interpret the sampling parameters:
    # 1. Direct interpretation: h_dim = int(hsampling[2]) - Used in _build_kvec_Brillouin()
    # 2. Range-based formula: hsteps = int(hsampling[2] * (hsampling[1] - hsampling[0]) + 1) - Used in map_utils.generate_grid()
    #
    # This class tries to maintain consistency by prioritizing the dimensions calculated by map_utils.generate_grid()
    # and stored in self.map_shape. When using explicit q-vectors mode, ensure they match what would be
    # generated by map_utils.generate_grid() with the same sampling parameters.
    ...
    def _build_kvec_Brillouin(self):
        ...
        # Check if we need to recompute dimensions based on map_shape
        if hasattr(self, 'map_shape') and self.map_shape is not None:
            # Get dimensions from map_shape instead of reinterpreting sampling parameters
            logger.info(f"Using dimensions from map_shape: {self.map_shape}")
            h_dim, k_dim, l_dim = self.map_shape
            # Log both approaches for comparison and debugging
            direct_h_dim = int(self.hsampling[2]) if self.hsampling else 0
            direct_k_dim = int(self.ksampling[2]) if self.ksampling else 0 
            ...
            # Ensure we're using the same dimensions as in compute_gnm_phonons
            total_points = h_dim * k_dim * l_dim
            logger.info(f"Using total_points = {total_points} from map_shape")
        else:
            # Fallback to direct interpretation if map_shape is not available
            h_dim = int(self.hsampling[2])
            k_dim = int(self.ksampling[2])
            l_dim = int(self.lsampling[2])
            total_points = h_dim * k_dim * l_dim
            logger.info(f"Using direct dimensions: {h_dim}x{k_dim}x{l_dim} = {total_points} points")
        ...
        if using_arbitrary_q:
            ...
            return
        if not hasattr(self, 'map_shape') or self.map_shape is None:
            # Fallback to direct interpretation if map_shape is not available
            h_dim = int(self.hsampling[2])
            k_dim = int(self.ksampling[2])
            l_dim = int(self.lsampling[2])
            logger.info(f"Using direct dimensions: {h_dim}x{k_dim}x{l_dim} = {h_dim*k_dim*l_dim} points")
    ...
    def compute_gnm_phonons(self):
        ...
        # Log map_shape vs direct dimension calculation
        if hasattr(self, 'map_shape'):
            map_shape_total = self.map_shape[0] * self.map_shape[1] * self.map_shape[2]
            logger.info(f"compute_gnm_phonons: map_shape={self.map_shape}, total points={map_shape_total}")
        
        # Log direct dimension calculation
        direct_h = int(self.hsampling[2]) if self.hsampling else 0
        direct_k = int(self.ksampling[2]) if self.ksampling else 0
        direct_l = int(self.lsampling[2]) if self.lsampling else 0
        direct_total = direct_h * direct_k * direct_l
        logger.info(f"compute_gnm_phonons: Direct dimensions: {direct_h}x{direct_k}x{direct_l}={direct_total}")
        ...
        if using_arbitrary_q:
            ...
            # Use the dimensions from map_shape (as generated by generate_grid in _setup)
            if hasattr(self, 'map_shape') and self.map_shape is not None:
                h_dim, k_dim, l_dim = self.map_shape
                logger.info(f"Using map_shape dimensions: {h_dim}x{k_dim}x{l_dim} = {h_dim*k_dim*l_dim} points")
            else:
        ...
        # Log before reshape operation
        logger.info(f"compute_gnm_phonons: Before reshape, Kmat_all shape: {Kmat_all.shape}")
        logger.info(f"compute_gnm_phonons: Attempting reshape to: ({total_points}, {self.n_asu * self.n_dof_per_asu}, {self.n_asu * self.n_dof_per_asu})")
    ...
    def apply_disorder(self, rank: int = -1, outdir: Optional[str] = None, 
        ...
        if using_arbitrary_q:
            ...
            if hasattr(self, 'map_shape') and self.map_shape is not None:
                h_dim, k_dim, l_dim = self.map_shape
                logger.info(f"Using map_shape dimensions: {h_dim}x{k_dim}x{l_dim}")
            else:
                # Fallback to direct interpretation
                h_dim = int(self.hsampling[2])
                k_dim = int(self.ksampling[2])
                l_dim = int(self.lsampling[2])
                logger.info(f"Using direct dimensions: {h_dim}x{k_dim}x{l_dim}")
    ...
    def _flat_to_3d_indices(self, flat_indices: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # For grid-based approach, calculate dimensions consistently
        if hasattr(self, 'map_shape') and self.map_shape is not None:
            # Use map_shape dimensions for consistency

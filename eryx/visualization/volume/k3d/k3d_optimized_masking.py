...
def create_final_optimized_masking(
    ...
    plot = k3d.plot(
        ...
        label_color=0x1e1e1e  # Match background to hide tick numbers (workaround)
    )
    plot.grid = bounds
    ...
    // Listen for commands from parent frame (for multi-panel control)
    if (window.parent !== window) {{
        window.addEventListener('message', function(e) {{
            ...
            switch(e.data.action) {{
                case 'get_camera':
                    if (k3dPlot && k3dPlot.camera) {{
                        window.parent.postMessage({{
                            type: 'camera_position',
                            camera: k3dPlot.camera
                        }}, '*');
                    }}
                    break;
                    
                case 'set_camera':
                    if (k3dPlot && e.data.camera) {{
                        k3dPlot.setCamera(e.data.camera);
                        k3dPlot.render();
                    }}
                    break;
                ...
                case 'start_rotation':
                    if (window.startCameraRotation) window.startCameraRotation();
                    break;
                    
                case 'stop_rotation':
                    if (window.stopCameraRotation) window.stopCameraRotation();
                    break;
                    
                case 'update_mask':
                    if (e.data.mask_type === 'spherical' && window.updateSphericalRadius) {{
                        window.updateSphericalRadius(e.data.radius);
                    }} else if (e.data.mask_type === 'octant' && window.updateOctantSelection) {{
                        window.updateOctantSelection(e.data.selection);
                    }}
                    break;
                    
                case 'toggle_log_scale':
                    if (window.toggleLogScale) window.toggleLogScale(e.data.enabled);
                    break;
                    
                case 'update_alpha':
                    if (window.updateAlphaCoef) window.updateAlphaCoef(e.data.value);
                    break;
                    
                case 'toggle_slicing':
                    if (sliceAnimationFrame) {{
                        if (window.stopSliceAnimation) window.stopSliceAnimation();
                    }} else {{
                        if (window.startSliceAnimation) window.startSliceAnimation();
                    }}
                    break;
            }}
        }});
    ...
    window.addEventListener('load', async function() {{
        ...
        async function initK3D() {{
            try {{
                ...
                if (window.parent !== window) {{
                    window.parent.postMessage({{ type: 'k3d_ready' }}, '*');
                }}
